...
void FunkenSetupImplementStepstick() {
  ...
  fnk.listenTo("MSMODE", setMicroStepMode); // not implemented
  fnk.listenTo("MS", setMicroStepPins); // not implemented
...
/*
   TYPICAL: MS 0 0 1 or MS [0 or 1] [0 or 1] [0 or 1]
*/
void setMicroStepPins(char *c) {
  char *token = fnk.getToken(c);
  char *pin1 = fnk.getArgument(c);
  char *pin2 = fnk.getArgument(c);
  char *pin3 = fnk.getArgument(c);
...
/*
   TYPICAL: MSMODE 0 or MSMODE [0(full) or 1(half) or 2(quarter) or 3(eighth) or 4(sixteenth)]
*/
